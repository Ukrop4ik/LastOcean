	...
	public class Mail
	{
		...
		public virtual void FromHashtable (Hashtable hash)
		{
			if (hash != null)
			{
				...
				{
					DateTime.TryParseExact(hash["SendDate"].ToString(), "yyyy-MM-dd HH:mm:ss", CultureInfo.CurrentCulture, DateTimeStyles.None, out sendDate);
				}
				if (hash.ContainsKey("ReadDate") && hash["ReadDate"] != null && !string.IsNullOrEmpty(hash["ReadDate"].ToString()))
				{
					DateTime.TryParseExact(hash["ReadDate"].ToString(), "yyyy-MM-dd HH:mm:ss", CultureInfo.CurrentCulture, DateTimeStyles.None, out readDate);
				}
